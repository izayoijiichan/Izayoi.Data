    ...
    public class SearchCondition
    {
        ...
        public override string ToString()
        {
            return _opType == OpType.None
                ? $"{_connector} {(_enclosureL == char.MinValue ? string.Empty : _enclosureL)} {_fieldName} {_operator} {_value} {(_enclosureR == char.MinValue ? string.Empty : _enclosureR)}"
                : $"{_connector} {(_enclosureL == char.MinValue ? string.Empty : _enclosureL)} {_fieldName} {_opType} {_value} {(_enclosureR == char.MinValue ? string.Empty : _enclosureR)}";

            //return $"{nameof(Connector)}: {_connector}, L: {(_enclosureL == char.MinValue ? string.Empty : _enclosureL)}, {nameof(FieldName)}: {_fieldName}, {nameof(OpType)}: {_opType}, {nameof(Operator)}: {_operator}, {nameof(Value)}: {_value}, R: {(_enclosureR == char.MinValue ? string.Empty : _enclosureR)}";
        }
